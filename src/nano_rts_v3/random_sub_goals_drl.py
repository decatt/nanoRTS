...
class ActorCritic(nn.Module):
    ...
    @torch.no_grad()
    def get_state_code(self, states):
        states = states.permute((0, 3, 1, 2))
        policy_network = self.policy_network(states)
        x = F.softmax(self.policy_unit(policy_network),dim=-1)
...
class Agent:
    def __init__(self,net:ActorCritic) -> None:
        ...
        self.env = GameEnv([map_path for _ in range(self.num_envs)],rewards_wrights,max_steps = 5000, random_sub_goals=False)
    ...
    def sample_env(self, check=False):  
        ...
        while len(self.exps_list[0]) < self.num_steps:
            ...
            state_codes = self.net.get_state_code(torch.Tensor(self.obs))
            for i in range(self.num_envs):
                state_code = state_codes[i].item()
                if state_code in self.sub_goals:
                    rs[i][0] == rs[i][0] + 1.0
                    self.sub_goals.remove(state_code)
    ...
    def generate_grads(self):
        ...
        for i in range(mini_batch_number):
            ...
            if ratio_type == 0:
                ...
                ratio = torch.exp(mini_new_log_probs.sum(dim=-1)-mini_old_log_probs.sum(dim=-1))
            else:
                ratio = torch.exp(mini_new_log_probs-mini_old_log_probs)
