...
class Game:
    ...
    def get_obstacles(self)->set:
        ...
        for building_pos in self.building_pos:
    ...
    def execute_unit_action(self,unit_pos:int):
        ...
        if unit.current_action == 'move':
            return self.execute_move_unit(unit_pos)
        elif unit.current_action == 'harvest':
            return self.execute_harvest_unit(unit_pos)
        elif unit.current_action == 'return':
            return self.execute_return_unit(unit_pos)
        elif unit.current_action == 'produce':
            return self.execute_produce_unit(unit_pos)
        elif unit.current_action == 'attack':
            return self.execute_attack_unit(unit_pos)
    ...
    def run(self):
        ...
        for unit in list(self.units.values()):
            if unit.building_unit_type is not None:
                self.building_pos.append(unit.current_action_target)
                self.using_resource += unit.building_unit_type.cost
        self.game_time += 1
        r0 = 0
        r1 = 0
        done = False
