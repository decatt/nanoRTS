from agent import AI, WorkerRush
from game import Game
from wrapper import GameEnvs
...
class Agent:
    def __init__(self,net:ActorCritic) -> None:
        ...
        self.env = GameEnvs([map_path for _ in range(self.num_envs)],max_steps = 2000)
    ...
    def sample_env(self, check=False):  
        ...
        while len(self.exps_list[0]) < self.num_steps:
            ...
            for i in range(self.num_envs):
                ...
                oppe_action = self.oppenent.get_random_action_list(self.env.games[i].gs)
                game = self.env.games[i]
                action = [game.vector_to_action(vector_action)]
                action.extend(oppe_action)
                actions_list.append(action)
            next_obs, rs, done_n, infos = self.env.step(actions_list)
            ...
            for i in range(self.num_envs):
                ...
                self.exps_list[i].append([self.obs[i],vector_actions[i],rs[i],mask[i],done,log_prob[i]])
                if check:
                    if done_n[i]:
                        if infos[i] == "player0":
